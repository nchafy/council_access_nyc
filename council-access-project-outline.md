...
2. **No composite "importance" / "salience" / "relevance" score for a neighbourhood.** Its heaviest proposed input varies 54.2x across statutorily equal-population districts, two proposed inputs correlate at ρ 0.892, the "friction" sub-score's own parts correlate at −0.704, and the rescale-to-100 step converts a measured #1-vs-#2 gap of exactly 0.000 into a 100-vs-99 headline. **Amended 2026-09-21 — this does not forbid the P11 shortlist.** Ranking the Council's *own observable activity* (items on an upcoming agenda, hearing counts, pipeline stage, repeated lay-overs, divided recorded votes) measures the institution from its published record and is in scope; inferring residents' priorities is not, and no heading may imply it. v1 orders items and states the reason in words beside each; it emits no 0-100 number. If a composite is ever added it requires the sensitivity analysis in §5 and must still display its components. Product definition: `product-brief.md` §4 P11 and §7.
...
| R30 | **Amended 2026-09-22 on measurement.** Publish any well-formed `cb_office_email`; never render `cb_chair` or `cb_district_manager`. The original strict rule (`^(mn\|bx\|bk\|qn\|si)\d{2}@cb\.nyc\.gov$` only) matches just **31 of 59** boards; the rest include board-domain mailboxes (`info@brooklyncb6.org`, `bronxcb6@bronxcb6.org`). A middle rule suppressing person-named locals was also tried and rejected: all 12 it caught are the district manager's **work** address on an official domain (11 on `cb.nyc.gov`), published by the City precisely so residents can make contact. The harm worth avoiding is the name-to-mailbox *pairing*, and declining to render the names prevents that. `dy27-rrad` (2019) and `3gkd-ddzn` (2017) remain blocklisted by dataset id. | MUST | `[ADD]` |
| R31 | The council-district → community-district mapping is built from geometry (`5crt-au7u` against `872g-cjhh`) at build time, **never** from `ruf7-3wgc.council_district` — that column carries only 44 distinct values across 59 rows and omits districts 5, 8, 15, 20, 27, 32 and 40, because it records the board *office's* district rather than the districts it covers. **Implemented 2026-09-22** as a committed crosswalk (`crosswalks/council_to_boards.json`) regenerated by `showup crosswalk`, because the sweep takes ~35 s and the boundaries change about once a decade; committing it also makes a boundary change a reviewable diff. A floor asserts all 51 districts resolve to ≥1 board. | MUST | `[ADD]` |
...
| R38 | Ship CSP with an explicit `connect-src` allowlist (`self`, `geosearch.planninglabs.nyc`), `Referrer-Policy: no-referrer`, `<meta name="referrer" content="no-referrer">`, and `rel="noopener noreferrer"` on every outbound link. Legistar's MeetingDetail embeds AddThis; never iframe or embed it. **Implemented 2026-09-22 without the `self`, and that was a live defect until 2026-09-23** — three same-origin fetches were refused, so local address resolution fell through to the geocoder and the staleness notice never rendered. The requirement was right and the implementation diverged from it in one token, which is the argument for `tests/unit/test_csp.py` cross-checking the served policy against the code rather than against this row. | MUST | `[ADD]` |
| R39 | WCAG 2.2 AA. All procedural content works with JavaScript disabled (pages are pre-rendered). axe over every pre-rendered page in CI, plus a documented keyboard-only and screen-reader pass per release. Every map answer reachable from a keyboard-navigable list of all 51 districts. **⚠️ Met except the screen-reader pass (2026-09-23):** axe clean over all 114 pages with a committed negative control; 122 keyboard and accessibility-tree checks; the no-JavaScript path verified with script execution disabled in the browser rather than inferred from the markup. The screen-reader leg is recorded as NOT RUN in `docs/accessibility-pass.md` §4 — reading the accessibility tree is not listening. | MUST | `[ADD]` |
| R40 | Performance budget enforced in CI: initial HTML + CSS + district JSON under 60 KB gzipped; geometry and MapLibre load after first paint; **cold** first answer under 3 s on a throttled 3G / mid-tier Android profile. **✅ Met (2026-09-23):** 13.7 KB heaviest page, 6.5 KB of script against a separate 12 KB ceiling, 132 KB geometry fetched only on the street-address path, 906 ms cold-load p95 on 3G Fast with a 4× CPU throttle. Thresholds in the committed `perf-budget.json`. | MUST | `[ADD]` |
...
**Amended 2026-09-21 — the shortlist (P11) ships here.** The owner's reason for wanting a score was triage: too much is in motion to follow. That is served by *ordering* items by the Council's own observable activity — on an upcoming agenda, hearing count and recency, pipeline stage (`Reported out of committee` means a floor vote is close), repeated `Laid Over by Committee`, divided recorded votes, time in play including re-introduction chains — and stating the reason beside each item in words. Sponsor count is a **candidate** signal only: `primary_sponsor` is a bare surname and co-sponsor data has not been verified as reachable. The shortlist is headed by what the Council is working on, never by what a neighbourhood cares about; that wording is a product decision, not a caption. It emits no composite number, so nothing above is weakened. Shortlist length and window are open question 5 in the brief.
...
| **M6** | Agenda items and recorded votes | **5** | `MeetingDetail` agenda items for the next 14 days with `class` + `class_reason` and the "agenda not yet published" state; Stated Meeting minutes vote parsing with the tally invariant and document-level quarantine; `resolve/member_names.py` at ≥98%; `bloc_concentration` published with the >0.60 copy withdrawal; `/committee/{slug}/` pages with rosters from dated PDF headers; **the P11 shortlist** — ordered by observable Council activity, one plain-language reason rendered beside every item, no composite number, at citywide / per-committee / per-member-and-so-per-district level. **First and only answer to question 2, and the answer to the triage need (J2b).** | "This is what the Council actually heard, here is a 41-9 vote with the nine named — and here are the eight things most in motion this month, each saying why it made the list." |
